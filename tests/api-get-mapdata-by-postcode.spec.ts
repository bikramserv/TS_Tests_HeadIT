...
test('Retrieve map data filtered by valid post code using GET endpoint', async () => {
  ...
  const validPostCode = '12345'; // TODO: Replace with a valid post code for your environment
  const response = await apiContext.get(`https://localhost:7203/api/MapDatas?postCode=${validPostCode}`);
